...
object Checker {
  ...
  def determineDefType(tree : Stmt) {
    tree.asInstanceOf[DefStmt] match {
      case t@DefStmt(_, name, maybeParams, hasRet, lst) => {
        ...
        if (t.isConstructor) {
          retType = resolveClassType(t.enclosingClass.getType(), tree)
        }
        if (t.sym == null) {
  ...
  def determineExprType(expr : Expression, cls : Stmt) {
    expr match {
      case FunExpr(name, param) => {
        ...
        if (retType.isInstanceOf[BoundClassSymbol]) {
          var ret = retType.asInstanceOf[BoundClassSymbol]
          var term = if (ret.cs.names != List()) Fun(ret.cs.name, ret.cs.names) else MVar(ret.cs.name)
          if (identSym.isInstanceOf[BoundClassSymbol]) {
            var newTerm = Unifier.subst(term, identSym.asInstanceOf[BoundClassSymbol].bindings)
            val bindings = identSym.asInstanceOf[BoundClassSymbol].bindings
            println("term = " + term + ", bindings = " + bindings + ", found newTerm = " + newTerm)
          }
          //expr.sym = newTerm
  ...
  def expectDeclSymbol(context : Context, cls : Stmt, str2 : String) : Option[DeclSymbol] = {
    ...
    println("searching context: " + context)
  ...
  def resolveClassType(t : Type, tree : Stmt) : BoundClassSymbol = {
    ...
    var name = ""
    var gens : List[Term] = List()

    t.full match {
      case Fun(n2, f1) => { name = n2; gens = f1 }
      case Bound(n1) => name = n1
      case _ => ;
    }
    ...
    println("resolving type: " + t)
    sym = tree.context.resolve{a : (Symbol,Context) => a._1 match {
      // search for name the same and run unifier to get type bindings
      case cs@ClassSymbol(n1,_) => {
        if (n1 == name) { bindings = Unifier.unifyTerms(gens, cs.names, List()); true } else false
      }
      case _ => false
    }}
